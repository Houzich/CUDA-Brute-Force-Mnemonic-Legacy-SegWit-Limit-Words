...
int stride_class::bruteforce_mnemonic(uint64_t grid, uint64_t block) {
	gl_bruteforce_mnemonic << <(uint32_t)grid, (uint32_t)block, 0, dt->stream1 >> > (dt->dev.words_pos, dt->dev.dev_tables_legacy, dt->dev.dev_tables_segwit, dt->dev.dev_tables_native_segwit, dt->dev.dev_wordlist, dt->dev.ret);
	return 0;
}

int stride_class::bruteforce_mnemonic_for_save(uint64_t grid, uint64_t block) {
	gl_bruteforce_mnemonic_for_save << <(uint32_t)grid, (uint32_t)block, 0, dt->stream1 >> > (dt->dev.words_pos, dt->dev.dev_tables_legacy, dt->dev.dev_tables_segwit, dt->dev.dev_tables_native_segwit, dt->dev.dev_wordlist, dt->dev.ret, dt->dev.mnemonic, dt->dev.hash160);
	return 0;
}
...
int stride_class::memsetGlobalMnemonic()
{
	...
	if (cudaMemcpyAsync(dt->dev.words_pos, dt->host.words_pos, dt->size_words_pos_buf, cudaMemcpyHostToDevice, dt->stream1) != cudaSuccess) { fprintf(stderr, "cudaMemcpyAsync to Board->dev.entropy failed!"); return -1; }
...
int stride_class::init()
{
	...
	for (int i = 0; i < 12; i++)
	{
		std::string name = "WordList " + tools::byteToHexString(i);
		if (dt->dev.cudaMallocDevice((uint8_t**)&dt->dev.wordlist[i].table, dt->host.wordlist[i].size, &memory_size, name.c_str()) != 0)
		{
			std::cout << "Error cudaMallocDevice(), Board->dev.WordList[" << i << "]!" << std::endl;
			return -1;
		}
		dt->dev.wordlist[i].size = dt->host.wordlist[i].size;
		dt->dev.memory_size += dt->host.tables_native_segwit[i].size;
	}
	std::cout << "MALLOC MEMORY SIZE (WORDLISTS GPU): " << std::to_string((float)memory_size / (1024.0f * 1024.0f)) << " MB\n";
	...
	for (int i = 0; i < 12; i++)
	{
		if (cudaMemcpy((void*)dt->dev.wordlist[i].table, dt->host.wordlist[i].table, dt->host.wordlist[i].size, cudaMemcpyHostToDevice) != cudaSuccess)
		{
			std::cout << "cudaMemcpy to Board->dev.wordlist[" << i << "] failed!" << std::endl;
			return -1;
		}
	}
	if (cudaMemcpy(dt->dev.dev_wordlist, dt->dev.wordlist, 12 * sizeof(wordlistStruct), cudaMemcpyHostToDevice) != cudaSuccess) { fprintf(stderr, "cudaMemcpyAsync to Board->dev.dev_wordlist failed!"); return -1; }
