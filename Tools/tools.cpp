...
namespace tools {
	...
	void generateRandomUint8Buffer(uint8_t* buff, size_t len, uint8_t limit) {
		...
		{
			*(uint16_t*)((uint16_t *)buff + i)= distr(eng);
		}
	...
	void generateRandomUint16Buffer(uint16_t* buff, size_t len, wordlistStruct *wordlists) {
		...
		for (int i = 0; i < len / 12; i++)
		{
			...
			{
				buff[i * 12 + ii] = buff[i * 12 + ii] % (wordlists[ii].size/2);
			}
		}
	...
	int getWordIndices(std::string str, uint16_t* gen_words_indices) 
	{
		for (int x = 0; x < 2047; x++) {
			if (strcmp(str.c_str(), (char*)&mnemonic_words[x]) == 0)
			{ 
				*gen_words_indices = x;
				return 0;
			}
	...
	void Test()
	{
		...
		entropy[0] = 0;
		entropy[1] = 0;
		int16_t gen_words_indices[12] = {-1};

		gen_words_indices[0] = 0;
		gen_words_indices[1] = 0;
		gen_words_indices[2] = 0;
		gen_words_indices[3] = 0;
		gen_words_indices[4] = 0;
		gen_words_indices[5] = 0;
		gen_words_indices[6] = 0;
		gen_words_indices[7] = 0;
		gen_words_indices[8] = 0;
		gen_words_indices[9] = -1;
		gen_words_indices[10] = 0;

		uint8_t mnemonic_phrase[120] = { 0 };
		for (int x = 0; x < 2047; x++) {
			entropyTo12Words(entropy, gen_words_indices, x, mnemonic_phrase);
			printf("%s\n", mnemonic_phrase);
		}
	...
	int checkResult(retStruct* ret) {
		...
		if (ret->f[0].count_found_bytes != 0)
		{
			for (uint32_t i = 0; i < ret->f[0].count_found_bytes; i++)
			{
				...
				std::string path = getPath(info->path, info->child);
				tools::encodeAddressBase58((const uint8_t*)info->hash160, addr);
				tools::encodeAddressBase58((const uint8_t*)info->hash160, addr_in_table);
		...
		if (ret->f[1].count_found_bytes != 0)
		{
			for (uint32_t i = 0; i < ret->f[1].count_found_bytes; i++)
			{
				...
				std::string path = getPath(info->path, info->child);
				tools::encodeAddressBIP49((const uint8_t*)info->hash160, addr);
				tools::encodeAddressBIP49((const uint8_t*)info->hash160, addr_in_table);
				std::cout << "\n!!!FOUND IN ADDRESS(HASH160) (" << path << ") EQUAL " << num_bytes << " BYTES: " << mnemonic_str << "," << addr << "," << addr_in_table << "," << hash160 << "," << hash160_in_table << " \n";
				tools::addInFileTest(path, mnemonic_str, hash160, hash160_in_table, addr, addr_in_table);
			}
		}
		if (ret->f[2].count_found_bytes != 0)
		{
			for (uint32_t i = 0; i < ret->f[2].count_found_bytes; i++)
			{
				...
				std::string path = getPath(info->path, info->child);
				tools::encodeAddressBase32((const uint8_t*)info->hash160, addr);
				tools::encodeAddressBase32((const uint8_t*)info->hash160, addr_in_table);
				std::cout << "\n!!!FOUND IN ADDRESS(HASH160) (" << path << ") EQUAL " << num_bytes << " BYTES: " << mnemonic_str << "," << addr << "," << addr_in_table << "," << hash160 << "," << hash160_in_table << " \n";
				tools::addInFileTest(path, mnemonic_str, hash160, hash160_in_table, addr, addr_in_table);
			}
		}
